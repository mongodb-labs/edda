...
def main():
    ...
    if len(namespace.filename) > 1:
        # for every pair of named servers, we can detect clock skew
        # between them
        logger.info("Attempting to resolve clock skew across servers")
        result = server_clock_skew(db, collName)
        logger.info('-' * 64)
        # for every unnamed server, check for events that line up
        # with or without clock skew, and estimate at the same time!
        logger.info("Attempting to resolve server names")
        result = server_matchup(db, collName)
    logger.warning('Exiting.')


def new_server(server_num, origin_server, db, collName):
