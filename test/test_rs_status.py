...
def test_process():
    ...
    # valid lines
    check_state("Mon Jun 11 15:56:16 "
    ...
    check_state("[rsSync] replSet member "
        "encountered FATAL ERROR", "FATAL", 4, "self")
    check_state("[rsStart] replSet STARTUP2", "STARTUP2", 5, "self")
    check_state("[rsSync] replSet member 10.4.3.56:45456 is now in"
        " state UNKNOWN", "UNKNOWN", 6, "10.4.3.56:45456")
    check_state("[rsHealthPoll] replSet member localhost:27019"
        " is now in state ARBITER", "ARBITER", 7, "localhost:27019")
    check_state("[rsHealthPoll] replSet member "
        "localhost:27017 is now in state DOWN", "DOWN", 8, "localhost:27017")
    check_state("[rsSync] replSet member example@domain.com:22234"
        " is now in state ROLLBACK", "ROLLBACK", 9, "example@domain.com:22234")
    check_state("[rsSync] replSet member my-MacBook-pro:43429 has been REMOVED"
        "", "REMOVED", 10, "my-MacBook-pro:43429")


def test_startup_with_IP():
...
def test_startup_with_hostname():
